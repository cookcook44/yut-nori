...
class Shape:
    ...
    def mal(self):
        if _utils.verified == 1:
            ...
            if _utils.wanna_move != -1: # 판에 놓는 위치 그거. 움직일 족보를 선택하고,
                if (_utils.selected == 4 and _utils.turn == 'R') or (_utils.selected == 5 and _utils.turn == 'B'): # 움직일 말을 선택하면,
                    if (_utils.turn == 'R' and _utils.left[0] >= 1) or (_utils.turn == 'B' and _utils.left[1] >= 1):
                        pygame.draw.circle(screen, (88, 48, 44), _utils.number_to_loc(_utils.number_to_yut(_utils.usable[_utils.wanna_move-1])[2]), 15) # 족보,말 선택하면 판에 띄워줌

            if _utils.selected == 6:
                pygame.draw.circle(screen, (88, 48, 44), _utils.number_to_loc(_utils.where_to_go(_utils.mal, _utils.number_to_yut(_utils.usable[_utils.wanna_move-1])[2], _utils.turn)), 15)
