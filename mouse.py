            ...
            if _utils.verified == 0:
                ...
                for i in range(len(_utils.usable)):
                    if self.check((650+i*105, 25), (80, 125), self.x, self.y) and (_utils.verified != 1 or _utils.who == _utils.turn):
                        _utils.wanna_move = i+1 # 움직일 족보 선택함
                        
                        
                if _utils.wanna_move != -1 and ((_utils.selected == 4 and _utils.turn == 'R') or (_utils.selected == 5 and _utils.turn == 'B')) and _utils.loc_to_number(self.x, self.y, _utils.turn) == _utils.number_to_yut(_utils.usable[_utils.wanna_move-1])[2] and _utils.loc_to_number(self.x, self.y, _utils.turn) != -1: # 누른 곳이 갈 수 있는 칸과 같으면 
                        ...
                        if boardloc != -1: # 처음에 놓는 것
                            if _utils.turn == 'R':
                                ...
                                if _utils.board[boardloc][0] == '_':
                                    ...
                                    _utils.board[boardloc] = ['R',1]
                                    if _utils.usable[_utils.wanna_move-1] != 0 and _utils.usable[_utils.wanna_move-1] != 4:
                                        _utils.rollable += 1
                                else:
                            ...
                            del _utils.usable[_utils.wanna_move-1]
                            print('초기화2')
                            _utils.wanna_move = -1
                            ...
                            def send_move():
                                msg_id = random.randint(1, 9989799)
                                for _ in range(10):
                                    # f:도착위치:남은말개수:메시지id
                                    msg = f"f:{boardloc}:{_utils.left[_utils.user-1]}:{msg_id}".encode("utf-8")
                ...
                elif _utils.wanna_move != -1:
                    boardloc = _utils.loc_to_number(self.x, self.y, _utils.turn) # boardloc는 현재 마우스 위치 -> 보드에서의 숫자
                    if (_utils.selected != 6 or boardloc != (_utils.where_to_go(_utils.mal,_utils.number_to_yut(_utils.usable[_utils.wanna_move-1])[2], _utils.turn))) and boardloc != -1 and _utils.board[boardloc][0] == _utils.turn and boardloc != 30 and boardloc != 31 and (_utils.verified != 1 or _utils.who == _utils.turn): # 자기 기물을 누름
                        _utils.selected = 6
                        _utils.mal = boardloc # 처음에 자기 기물 누를 때 _utils.mal에 boardloc를 저장해놓음 
                    elif _utils.selected == 6 and boardloc == (_utils.where_to_go(_utils.mal,_utils.number_to_yut(_utils.usable[_utils.wanna_move-1])[2], _utils.turn)): # 자기 기물을 누른 상태에서 족보 고르고 갈 곳 누름
                        ...
                        if _utils.turn == 'R':
                            if _utils.board[boardloc][0] == '_':
                                ...
                                if _utils.usable[_utils.wanna_move-1] != 0 and _utils.usable[_utils.wanna_move-1] != 4: # 모,윷으로 잡는건 +1 안됨
                                    _utils.rollable += 1
                            else:
                        ...
                        def send_move_existing(src, dest):
                            msg_id = random.randint(1, 98899669) 
                            for _ in range(10):
                                # m:출발위치:도착위치:남은말개수:메시지id
                                msg = f"m:{src}:{dest}:{_utils.left[_utils.user-1]}:{msg_id}".encode("utf-8") # f랑 m 구분!
                                sock.sendto(msg, _link.opp_addr)
                                pygame.time.delay(20)
                        thread7 = Thread(target=send_move_existing, args=(_utils.mal, boardloc))
                        thread7.start()
                        if len(_utils.usable) == 0 and _utils.rollable == 0:
                            _utils.turn_change()
